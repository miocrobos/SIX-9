...
const STARTER_CHIPS = [
  ...
  "Explain how trade settlement works",
  "Capture the compliance approval workflow",
]
...
export function AiSidebar({ isOpen, onClose, roomId, projectId }: AiSidebarProps) {
  ...
  // Feeds we've already created this session. A feed only needs to be created
  // once; caching avoids repeat createFeed calls that log noisy "already exists"
  // errors and add unnecessary feed-mutation load (a source of timeouts).
  const ensuredFeedsRef = useRef<Set<string>>(new Set())

  // Create the feed on demand before writing to it. Idempotent: a feed that is
  // already cached or reported as existing by the server is treated as ready.
  // Only genuine, unexpected failures are rethrown so callers can surface them.
  const ensureFeed = useCallback(
    async (feedId: string) => {
      if (ensuredFeedsRef.current.has(feedId)) return
      try {
  ...
  // Ensure both feeds exist on mount. "Already exists" is the expected case and
  // is handled quietly by ensureFeed.
  useEffect(() => {
    void ensureFeed(FEED_ID).catch(() => {})
    void ensureFeed(CHAT_FEED_ID).catch(() => {})
  ...
  const handleRunTerminal = useCallback(
    async (status: string, output: unknown) => {
      const isSuccess = status === "COMPLETED"
      const typedOutput = output as { summary?: string } | undefined
      const content = isSuccess
        ? (typedOutput?.summary ?? "Knowledge map updated.")
        : "Sense AI encountered an error. Please try again."

      await ensureFeed(CHAT_FEED_ID).catch(() => {})
      await ensureFeed(FEED_ID).catch(() => {})
    ...
    [createFeedMessage, updateMyPresence, ensureFeed]
  )
  ...
  const handleSend = useCallback(async () => {
    ...
    // Ensure feeds exist, then push the user message to the shared ai-chat feed
    await ensureFeed(CHAT_FEED_ID).catch(() => {})
    await ensureFeed(FEED_ID).catch(() => {})
    ...
    createFeedMessage(FEED_ID, {
      text: "Sense AI is analyzing your request…",
      status: "start",
    }).catch(() => {})

    setStatusText("Sense AI is analyzing your request…")
    ...
    try {
      ...
      createFeedMessage(CHAT_FEED_ID, {
        sender: "Sense AI",
        role: "assistant",
        content: "Failed to reach Sense AI. Please try again.",
        timestamp: new Date().toISOString(),
      }).catch(() => {})

      createFeedMessage(FEED_ID, {
        text: "Sense AI encountered an error.",
        status: "error",
      }).catch(() => {})
  ...
  }, [input, isLoading, roomId, projectId, updateMyPresence, createFeedMessage, ensureFeed, self])
  ...
  }, [chatInput, createFeedMessage, ensureFeed, self])
        ...
        >
          <DialogHeader>
            <DialogTitle className="pr-6 text-sm font-medium text-text-primary">
              {selectedSpec ? getFilename(selectedSpec.filePath) : "Brief Preview"}
            </DialogTitle>
          </DialogHeader>
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border-default px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-text-primary">Six Sense</p>
          <p className="text-xs text-text-muted">Collaborate with Sense AI</p>
        </div>
        {isLoading && (
                  ...
                  <div className="flex flex-col items-center gap-5 py-8 text-center">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-text-muted">
                        Describe a topic, process, or document and I&apos;ll map the knowledge onto the canvas.
                      </p>
                    </div>
            ...
            <div className="shrink-0 border-t border-border-default p-3">
              <div className="flex flex-col gap-2 rounded-2xl border border-border-subtle bg-bg-elevated p-3">
                <Textarea
                  ...
                  placeholder="Describe a topic, process, or document…"
                  disabled={isLoading}
          ...
          <div className="flex h-full flex-col gap-3 p-4">
            ...
            {specsLoading ? (
              ...
              <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
                ...
                <p className="text-xs text-text-muted">No briefs yet. Generate one above.</p>
              </div>
            ) : (
