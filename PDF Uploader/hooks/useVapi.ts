...
export function useVapi(book: IBook) {
    ...
    useEffect(() => {
        const handlers = {
            'call-start': () => {
                ...
                // If the session was started by typing, send the queued message now
                if (pendingTextRef.current) {
                    ...
                    try {
                        ...
                        console.error('Failed to send queued text message:', err);
                    }
                }
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            const firstMessage = `Hi, I'm here to help you tap into the knowledge captured in "${book.title}". What would you like to understand first?`;
    ...
    const sendText = useCallback(async (text: string) => {
        ...
        // No active session yet → start one and queue the message until the call connects
        ...
        // Session is active → optimistically render the message and send it
